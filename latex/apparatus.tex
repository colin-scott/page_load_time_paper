...
In replay mode, the WPR proxy responds to HTTP(S) requests with the responses saved in the archive, or a 404 error response if the corresponding response is not saved in the archive. Here, WPR is emulating an edge cache rather than a browser cache.
...
We use Telemetry to orchestrate one of two browsers. The first is a desktop version of Google Chrome running within a virtual machine (specifications in ~\ref{specs}). The second is a mobile version of Google Chrome running on a USB-tethered mobile device.
...
\end{figure*}
Before each experiment, we clear the browser cache to ensure consistency across experiments. For each device (desktop and mobile), we execute the following steps for each URL:
...
Next, we determine the page load time of the web page with a cold cache. With WPR in replay mode, we load the URL four times (for discussion see~\ref{known_limitations}) and take the minimum page load time as our PLT value, to account for variance.
...
Our mobile device is a Galaxy Tab 4 with a 1.2 GHz quad-core processor and 1.5 GB on board RAM running Android\textsuperscript{\texttrademark} 4.4, KitKat. Desktop results were performed in a virtual machine with a 3.2 GHz quad-core processor and 6 GB RAM.
